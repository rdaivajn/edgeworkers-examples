...
class Clipper {
    constructor(clipOptions) {
        this.clipQP = "clip", this.globalRegex = new RegExp(/^clip=(\d+-\d+$|\d+)$/), this.clipOptions = clipOptions || {
            method: ClippingMethod.SEGMENT_BOUNDARIES
        }, this.validateOptions(this.clipOptions);
    }
    validateOptions(clipOptions) {
        if (![ ClippingMethod.PRECISE, ClippingMethod.SEGMENT_BOUNDARIES ].includes(clipOptions.method)) throw new Error(`Invalid cliping method!, Expected '${ClippingMethod.PRECISE.toString()}' or '${ClippingMethod.SEGMENT_BOUNDARIES.toString()}'`);
    }
    clip(clipInfo, playList) {
        if (!playList || "object" != typeof playList || !("isMasterPlaylist" in playList)) throw Error("Invalid playList type, expected instance of master or media playlist");
        const clipingParams = this.validateClipInfo(clipInfo);
        return playList.isMasterPlaylist ? this.clipMasterPlaylist(clipingParams, playList) : this.clipMediaPlaylist(clipingParams, playList);
    }
    validateClipInfo(clipInfo) {
        if ("string" != typeof clipInfo) throw Error("Invalid clipInfo type, expected string");
        if (!this.globalRegex.test(clipInfo)) throw Error("Invalid clipInfo format, expected format clip=start[-end] where end is optional. e.g. clip=122-3224 or clip=122");
        const clipParams = (clipInfo = clipInfo.replace(this.clipQP + "=", "")).split("-");
        return 1 == clipParams.length ? {
            start: +clipParams[0]
        } : {
            start: +clipParams[0],
            end: +clipParams[1]
    ...
    clipMasterPlaylist(clipingParams, playList) {
        for (let i = 0; i < playList.variants.length; i++) if (-1 == playList.variants[i].uri.indexOf("?")) {
            const urlSearchParams = new URLSearchParams;
            urlSearchParams.set(this.clipQP, clipingParams.end ? clipingParams.start.toString() + "-" + clipingParams.end.toString() : clipingParams.start.toString()), 
            playList.variants[i].uri = playList.variants[i].uri + "?" + urlSearchParams.toString();
        } else {
            const index = playList.variants[i].uri.indexOf("?"), urlSearchParams = new URLSearchParams(playList.variants[i].uri.slice(index + 1, playList.variants[i].uri.length));
            urlSearchParams.set(this.clipQP, clipingParams.end ? clipingParams.start.toString() + "-" + clipingParams.end.toString() : clipingParams.start.toString()), 
            playList.variants[i].uri = playList.variants[i].uri.slice(0, index) + "?" + urlSearchParams.toString();
        }
        return playList;
    }
    clipMediaPlaylist(clipingParams, playList) {
        let index = 0, clippedSegStartIndex = -1, clippedSegEndIndex = clipingParams.end ? -1 : playList.segments.length - 1, duration = 0, timeOffset = 0;
        for (;index < playList.segments.length && -1 == clippedSegStartIndex; ) duration += playList.segments[index].duration, 
        duration > clipingParams.start && (clippedSegStartIndex = index, this.clipOptions.method == ClippingMethod.PRECISE && (timeOffset = playList.segments[index].duration - (duration - clipingParams.start), 
        playList.start = {
            offset: timeOffset,
            precise: !0
        })), index++;
        if (clipingParams.end) for (;index < playList.segments.length && -1 == clippedSegEndIndex; ) duration += playList.segments[index].duration, 
        duration >= clipingParams.end && (clippedSegEndIndex = index), index++;
        clipingParams.end && -1 == clippedSegEndIndex && (clippedSegEndIndex = playList.segments.length - 1), 
        playList.segments.splice(0, clippedSegStartIndex);
        const rE = clippedSegEndIndex - clippedSegStartIndex;
        return playList.segments.splice(rE + 1, playList.segments.length - rE), playList;
    }
}

hlsParser.setOptions({
    strictMode: !0
});
...
class HLS {
    ...
    static insertAuxiliaryContent(playlistObj, bumpers) {
        ...
        for (const key in bumpers) {
            bumpers[key].auxiliaryPlayist.segments[0].discontinuity = !0;
        }
        let auxCntSegListLen = 0;
        for (let i = 0; i < bumpersListSize; i++) auxContentPosInPrimPlaylistArr[i] += auxCntSegListLen, 
        playlistObj.segments.splice(auxContentPosInPrimPlaylistArr[i], 0, ...bumpers[i].auxiliaryPlayist.segments), 
        auxCntSegListLen += bumpers[i].auxiliaryPlayist.segments.length;
    }
}

var types$2 = hlsParser.types;

export { Clipper, ClippingMethod, HLS, ManifestType, types$2 as types };
